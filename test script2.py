...
# classification
classification_data = pd.read_csv("megastore-classification-dataset - Copy.csv")
...
for col in X_test_cat.columns:
    X2[col].fillna(statistics.at[col, 'mode'])

Y2.fillna(statistics.at['ReturnCategory', 'mode'])
...
hh = encoder1.inverse_transform(Y2)
